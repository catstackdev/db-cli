...
adapter::tables_plain() {
  _pg::need || return 1
  _pg::exec -tAc "SELECT tablename FROM pg_tables WHERE schemaname='$DB_SCHEMA'"
}
...
adapter::sample() {
  _pg::need || return 1
  db::valid_id "$1" || return 1
  _pg::exec -c "SELECT * FROM $1 LIMIT ${2:-10}"
}

adapter::count() {
  _pg::need || return 1
  db::valid_id "$1" || return 1
  _pg::exec -tAc "SELECT COUNT(*) FROM $1"
}
...
adapter::export() {
  ...
  case "$fmt" in
    csv)  _pg::exec -c "COPY ($sql) TO STDOUT WITH CSV HEADER" > "$out" ;;
    json) _pg::exec -tAc "$sql" --json > "$out" ;;
...
adapter::copy() {
  ...
  _pg::exec -c "CREATE TABLE $2 AS SELECT * FROM $1" && db::ok "copied: $1 -> $2"
}

adapter::truncate() {
  _pg::need || return 1
  db::valid_id "$1" || return 1
  _pg::exec -c "TRUNCATE TABLE $1" && db::ok "truncated: $1"
}
